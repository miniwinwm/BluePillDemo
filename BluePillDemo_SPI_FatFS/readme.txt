...
file system on a SD card. This is the bestmethod of accessing an external file system on the Blue
...
This example does not set the real time clock and hemce uses a fixed time for file creation time
and date. It is possiblt also to enable the RTC module and have proper file creation timestamps
once the RTC has been initialized.

To conenct to the SD card a cheap SD card slot with SPI connections is used.
